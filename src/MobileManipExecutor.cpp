...
MobileManipExecutor::MobileManipExecutor(MotionPlan &currentMotionPlan) {
    ...
    std::cout << " Arm motion profile of " << vvd_arm_motion_profile[0].size() << " joints and " << vvd_arm_motion_profile.size() << " samples is loaded " << std::endl;
}
...
void MobileManipExecutor::getArmCommand(Joints& j_next_arm_command) {
	...
	else
	{
		...
		{
			this->vd_next_arm_config.push_back(j_next_arm_command.m_jointStates[i].m_position);
