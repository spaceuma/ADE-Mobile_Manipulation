...
int FetchingPoseEstimator::getFetchWaypointIndex(const std::vector<base::Waypoint> *roverPath)
{
    ...
    {
        double distance = sqrt(pow((*roverPath)[i].position[0] - samplePos.position[0], 2)
        ...
        if (distance < maxFetchingDistance && distance > minFetchingDistance)
        {
            ...
            cost += getHeadingCost((*roverPath)[i], (*roverPath)[roverPath->size() - 1]);
            if (cost < minCost)
            {
                minCost = cost;
...
{
    double cost = 2 * abs(distance - (maxFetchingDistance + minFetchingDistance) / 2)
                  / (maxFetchingDistance - minFetchingDistance);
    return cost;
}

double FetchingPoseEstimator::getHeadingCost(base::Waypoint pathPose, base::Waypoint samplePose)
{
    double bestHeading
        = atan2(samplePose.position[1] - pathPose.position[1], samplePose.position[0] - pathPose.position[0]);
