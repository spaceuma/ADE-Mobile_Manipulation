                                              ...
                                              base::Waypoint sample)
{
    ...
    this->fmPlanner.planPath(&costMap,
...
int MotionPlan::shortenPathForFetching()
{
    ...
    int endWaypoint
        = this->fetchPosePlanner.getFetchWaypointIndex(&(this->roverPath));
    std::cout << "MotionPlan: the end waypoint is " << endWaypoint << std::endl;
    this->roverPath.erase(this->roverPath.begin() + endWaypoint + 1,
                          this->roverPath.end());
    return endWaypoint;
}
                                            ...
                                            double zResolution)
{
    ...
    /*for ( uint j = 0; j < elevationMap.size(); j++ ){
            ...
            std::cout << " Cost map at " << i << ", " << j << " is " <<
    elevationMap[j][i] << std::endl;
        }
    }*/
    /*for (uint i = 0; i < this->roverPath.size(); i++)
    {
    std::cout << "Waypoint " << i << " is ( " << this->roverPath[i].position[0]
    << ", " << this->roverPath[i].position[1] << " )" << std::endl;
    }*/
    std::cout << " The resolution is " << inputMap->getResolution()
              << std::endl;
    std::cout << "The sample position is " << this->samplePos.position[0] << ","
              << this->samplePos.position[1] << std::endl;
    this->armPlanner.planArmMotion(&(this->roverPath),
                                   &elevationMap,
                                   inputMap->getResolution(),
                                   zResolution,
                                   this->samplePos,
    ...
    std::vector<std::vector<double>> &m_arm_motion_profile)
{
    ...
    std::cout << "Setting Arm Motion Profile of "
              << m_arm_motion_profile[0].size() << " joints and "
              << m_arm_motion_profile.size() << " samples" << std::endl;
    this->vvd_arm_motion_profile.clear();
...
void MotionPlan::setPath(std::vector<Waypoint> &vw_path)
{
    ...
    std::cout << "Setting Path of " << vw_path.size() << " waypoints"
              << std::endl;
