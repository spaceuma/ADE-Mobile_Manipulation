...
for testNum in range(1,41):
    if testNum < 10:
        armJoints = np.loadtxt(open("../../test/unit/moveit/results/profile0"+str(testNum)+".txt",'r'), skiprows=0)
        path = np.loadtxt(open("../../test/unit/moveit/results/path0"+str(testNum)+".txt"), skiprows=0)
        execTime = np.loadtxt(open("../../test/unit/moveit/results/computationTime0"+str(testNum)+".txt"), skiprows=0)
    else:
        armJoints = np.loadtxt(open("../../test/unit/moveit/results/profile"+str(testNum)+".txt",'r'), skiprows=0)
        path = np.loadtxt(open("../../test/unit/moveit/results/path"+str(testNum)+".txt"), skiprows=0)
        execTime = np.loadtxt(open("../../test/unit/moveit/results/computationTime"+str(testNum)+".txt"), skiprows=0)


    pathSize = np.size(path, 0)
    profileSize = np.size(armJoints, 0)
    ...
    for i in range(1, profileSize):
        ...
        if np.max(armJointsDist) > 0:
            totalDistToCollisions = totalDistToCollisions + armMovementTime*(prevDistToCollision + currDistToCollision)/2
    ...
    extraTimeFinalSpotTurn = 45/8.6 + 5; # Rotational velocity x avg turn + steering positioning time
    totalBaseRequiredTime = totalBaseRequiredTime + extraTimeFinalSpotTurn

    totalRequiredTime = np.max([totalArmRequiredTime, totalBaseRequiredTime])
    ...
    execTime = execTime + 1.5 # Add time for costmap computation
    ...
    if not os.path.exists("motionPlanResultsLog_moveIt.txt"):
        with open("motionPlanResultsLog_moveIt.txt", "a+") as file_object:
            for i in range(0, 8):
                file_object.write("\n")

    with open("motionPlanResultsLog_moveIt.txt", "r") as file_object:
        file_text = file_object.readlines()

    # Update the text
    file_text[0] = file_text[0][:-1]
    file_text[0] += " " + str(execTime) + "\n"

    file_text[1] = file_text[1][:-1]
    file_text[1] += " " + str(stdDevDistToCollisions) + "\n"

    file_text[2] = file_text[2][:-1]
    file_text[2] += " " + str(avgDistToCollisions) + "\n"

    file_text[3] = file_text[3][:-1]
    file_text[3] += " " + str(totalArmRequiredTime) + "\n"

    file_text[4] = file_text[4][:-1]
    file_text[4] += " " + str(totalBaseRequiredTime) + "\n"

    file_text[5] = file_text[5][:-1]
    file_text[5] += " " + str(totalArmRequiredTime) + "\n"

    file_text[6] = file_text[6][:-1]
    file_text[6] += " " + str(totalBaseRequiredTime) + "\n"

    file_text[7] = file_text[7][:-1]
    file_text[7] += " " + str(totalRequiredTime) + "\n"
