...
int main(int argc, char *argv[])
{
    ...
    KinematicModel_lib::Manipulator *manip
        = new KinematicModel_lib::Manipulator(s_urdf_path);
    ArmPlanner_lib::ArmPlanner *arm_planner
        = new ArmPlanner_lib::ArmPlanner(s_urdf_path);
    CollisionDetector *collision_detector = new CollisionDetector(s_urdf_path);

    std::vector<std::vector<double>> eeTransform
        = manip->getEETransform(lastConfiguration);

    // Point 2
    std::vector<double> position = {
        eeTransform[0][3] + 0.125, eeTransform[1][3] - 0.1, eeTransform[2][3]};
    std::vector<double> orientation = {pi, 0, 0};

    // nextConfiguration = manip->getManipJoints(position, orientation,1,1);
    nextConfiguration
        = manip->getManipJoints(position, orientation, lastConfiguration);

    if (collision_detector->isColliding(nextConfiguration))
    ...
    // Point 4
    position
        = {eeTransform[0][3] - 0.125, eeTransform[1][3], eeTransform[2][3]};
    ...
    std::vector<double> timeProfile
        = arm_planner->getTimeProfile(&armSweepingProfile);
