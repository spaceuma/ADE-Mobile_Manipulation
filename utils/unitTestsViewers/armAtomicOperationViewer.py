...
stopx = xsize*res+minValues[0]
stopy = ysize*res+minValues[1]
...
for i in range(0,n,round(n/5)):
  T = DKM(armJoints[i,:], [0,0,0], [0,0,0])
  rotT = T
  rotT[0,3] = 0
  rotT[1,3] = 0
  rotT[2,3] = 0

  Tbx = traslation([1,0,0])
  Tby = traslation([0,1,0])
  Tbz = traslation([0,0,1])

  Tx = dot(rotT, traslation([1,0,0]))
  Ty = dot(rotT, traslation([0,1,0]))
  Tz = dot(rotT, traslation([0,0,1]))

  px,py,pz = plotArm(armJoints[i,:], [0,0,0], [0,0,0])
  px = np.array(px)
  py = np.array(py)
  pz = np.array(pz)
...
T = DKM(armJoints[n-1,:], [0,0,0], [0,0,0])
rotT = T
rotT[0,3] = 0
rotT[1,3] = 0
rotT[2,3] = 0
...
px,py,pz = plotArm(armJoints[n-1,:], [0,0,0], [0,0,0])
px = np.array(px)
py = np.array(py)
pz = np.array(pz)
