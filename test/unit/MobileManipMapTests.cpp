...
TEST(MMMapTest, nominal_working_test_spacehall)
{
    ...
    ASSERT_NO_THROW(readMatrixFileCommas("test/unit/data/input/MMMapTest/RgDem.p_heightData_m.csv",
                                         vvd_elevation_data))
        << "Input DEM file is missing";

    ASSERT_NO_THROW(readMatrixFileCommas(
        "test/unit/data/input/MMMapTest/RgDem.p_pointValidityFlag.csv", vvd_validity_data))
        << "Input DEM file is missing";
    std::cout << "Matrix size is " << vvd_elevation_data[0].size() << "x"
              << vvd_elevation_data.size() << std::endl;
    std::cout << "Matrix size is " << vvd_validity_data[0].size() << "x" << vvd_validity_data.size()
              << std::endl;
    ...
    for(uint j = 0; j < vvd_elevation_data.size(); j++)
    {
        for(uint i = 0; i < vvd_elevation_data[0].size(); i++)
        {
            ...
            prgd_dummy_dem->p_pointValidityFlag[i + j * vvd_elevation_data[0].size()] =
                vvd_validity_data[j][i];
        }
    }
    ...
    writeMatrixFile("test/unit/data/results/MMMapTest/spacehall_traversabilityMap.txt",
                    traversabilityMap);
}
...
TEST(MMMapTest, nominal_working_test_galopprennbahnwest)
{
    ...
    ASSERT_NO_THROW(readMatrixFile(
        "test/unit/data/input/MMMapTest/GalopprennbahnWest_Zone01blurred_10cmDEM.csv",
        vvd_elevation_data))
        << "Input DEM file is missing";
    double res = 0.1;    // meters
    RoverGuidance_Dem * prgd_dummy_dem = new RoverGuidance_Dem;
    ...
    writeMatrixFile("test/unit/data/results/MMMapTest/gbwest_traversabilityMap.txt",
                    traversabilityMap);
}
...
TEST(MMMapTest, nominal_working_test_exrColmenar)
{
    ...
    cv::FileStorage opencv_file(
        "test/unit/data/input/MMMapTest/1212.1628281354.363863_colmenar_subdem_id_1.ext",
        cv::FileStorage::READ);
    ...
    // cv::Mat mGlobalDEM =
    // cv::imread("test/unit/data/input/MMMapTest/1212.1628281354.363863_colmenar_subdem_id_1ag.ext",
    // cv::IMREAD_ANYDEPTH);
    ...
    for(uint j = 0; j < ui_rows; j++)
    {
        ...
        {
            prgd_dummy_dem->p_heightData_m[i + j * ui_cols] = exrDEM.at<float>(j, i);
    ...
    writeMatrixFile("test/unit/data/results/MMMapTest/exrcolmenar_traversabilityMap.txt",
                    traversabilityMap);
}

TEST(MMMapTest, nominal_working_test_RH1)
{
    // Input Elevation Matrix is read
    std::vector<std::vector<double>> vvd_elevation_data, vvd_validity_data;
    ...
    writeMatrixFile("test/unit/data/results/MMMapTest/RH1_validityMap.txt", validityMap);
    writeMatrixFile("test/unit/data/results/MMMapTest/RH1_costMap.txt", costMap);
    writeMatrixFile("test/unit/data/results/MMMapTest/RH1_slopeMap.txt", slopeMap);
    writeMatrixFile("test/unit/data/results/MMMapTest/RH1_sdMap.txt", sdMap);
    writeMatrixFile("test/unit/data/results/MMMapTest/RH1_traversabilityMap.txt",
                    traversabilityMap);
    ASSERT_EQ(dummyMap.computeFACE(samplePos, roverPos), 0);
    std::cout << "Cost map is computed" << std::endl;
    double d_elevation_min = dummyMap.getMinElevation();
...
{
    cv::Mat mGlobalDEM =
        cv::imread("test/unit/data/input/MMMapTest/MagDEMs/00600.exr", cv::IMREAD_ANYDEPTH);
    cv::Mat mLocalDEM =
        cv::imread("test/unit/data/input/MMMapTest/MagLocCam/00600.exr", cv::IMREAD_ANYDEPTH);
...
TEST(MMMapTest, nominal_working_test_MagDEMs)
{
    ...
    for(uint j = 0; j < 400; j++)
    {
        ...
        {
            prgd_dummy_dem->p_heightData_m[i + j * 400] = dem01.at<float>(j, i) * 0.001;
...
TEST(MMMapTest, nominal_working_test_2)
{
    ...
    std::vector<std::vector<double>> vvd_elevation_data, vvd_validity_data;
    ASSERT_NO_THROW(readMatrixFile(
        "test/unit/data/input/MMMapTest/ColmenarRocks_splitted_10cmDEM.csv", vvd_elevation_data))
        << "Input DEM file is missing";
    double res = 0.1;    // meters
...
TEST(MMMapTest, sample_pos_error_test)
{
    ...
    std::cout << "\033[32m[----------]\033[0m [INFO] Testing error with waypoint "
              ...
              << std::endl;
    ASSERT_EQ(dummyMap.computeFACE(w_sample_one, roverPos), 2);
    std::cout << "\033[32m[----------]\033[0m [INFO] Testing error with waypoint "
                 "out of range - Second Sample"
              << std::endl;
    ASSERT_EQ(dummyMap.computeFACE(w_sample_two, roverPos), 2);
    std::cout << "\033[32m[----------]\033[0m [INFO] Testing error with waypoint "
                 "out of range - Third Sample"
              << std::endl;
    ASSERT_EQ(dummyMap.computeFACE(w_sample_three, roverPos), 2);
    std::cout << "\033[32m[----------]\033[0m [INFO] Testing error with waypoint "
                 "out of range - Fourth Sample"
              << std::endl;
    ASSERT_EQ(dummyMap.computeFACE(w_sample_four, roverPos), 2);
}
