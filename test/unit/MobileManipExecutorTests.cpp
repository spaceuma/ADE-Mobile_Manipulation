...
TEST(MMExecutorTest, trajectorycontrol)
{
    ...
    /*    while (!dummyExecutor.isFinished())
       ...
       "deg" << std::endl;
                robotRot = AngleAxisd(mc.m_turnRate_rads * dt,
       ...
       robotPose.orientation;
            ...
            else if (fabs(mc.m_turnRate_rads) < 0.000001)
            {
                ...
                robotPose.position += (mc.m_speed_ms * dt) * (toWCF *
       Eigen::Vector3d::UnitX());
            ...
            else
            {
                ...
                robotRot = AngleAxisd(mc.m_turnRate_rads * dt,
       Eigen::Vector3d::UnitZ());
                robotPose.position = robotRot * (robotPose.position -
       ...
       j_current_joints.m_jointStates[i].m_position << std::endl;
        }
        std::cout << std::endl;
            usleep(10000);
        }
