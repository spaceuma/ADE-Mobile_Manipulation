...
TEST(MMExecutorTest, armnotworking_test)
{
    ...
    robotSimPoseFile.open(
        "test/unit/data/results/MMExecutorTest/roverEstimatedPos_armnotworking.txt");
    ...
    while(ui_error_code != 2)
    {
        ...
        if(ui_loop_counter > 201)
        {
            ASSERT_EQ(ui_error_code, 6)
                << "\033[31m[----------]\033[0m Motion Command is ( transl = " << mc.m_speed_ms
                << ", rot = " << mc.m_turnRate_rads << ") ";
            break;
        }
